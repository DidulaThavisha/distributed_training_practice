...
class Head(nn.Module):
    ...
    def __init__(self, head_size: int, n_embed: int, context_length: int) -> None:
        ...
        self.register_buffer('tril', torch.tril(torch.ones(self.context_length, self.context_length, dtype=torch.bool)))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_weights1 = q @ k.transpose(-2, -1) * scale_factor
        # Apply causal masking
        # attn_weights2 = attn_weights1.masked_fill(self.tril[:T, :T] == 0, float('-inf'))
        self.tril = torch.tril(torch.ones(self.context_length, self.context_length, dtype=torch.bool, device=x.device))
        attn_weights2 = attn_weights1.masked_fill(self.tril[:T, :T]==0, float('-inf'))
        attn_weights3 = F.softmax(attn_weights2, dim=-1)
        v = self.value(x)   # (B, T, head_size)
        # Apply attention weights to values
        out = attn_weights3 @ v # (B, T, T) @ (B, T, head_size) -> (B, T, head_size)
        return out
